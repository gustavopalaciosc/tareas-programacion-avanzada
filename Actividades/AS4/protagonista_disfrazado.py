...
class ProtagonistaDisfrazado: 
    ...
    def recuperar_casas_BFS_iterativo(self, casa_inicial: Casa)-> list:
        ...
        while len(cola) > 0:
            ...
            ans.append({"casa": vertice, "distancia": vertice.distancia})
 
              

            if vertice.hijo_izquierdo is not None:
                if vertice.hijo_izquierdo not in visitados:
                    vertice.hijo_izquierdo.distancia += vertice.distancia
                    cola.append(vertice.hijo_izquierdo)

            if vertice.hijo_derecho is not None:
                    if vertice.hijo_derecho not in visitados:
                        vertice.hijo_derecho.distancia += vertice.distancia
